...
// PostgreSQL connection - Auth Database (Isolated Instance)
const pool = new Pool({
...
// Direct auth endpoints (temporary until Java service is fixed)
app.post("/auth/register", authLimiter, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (email, password, first_name, last_name, phone_number, role, enabled)
...
app.post("/auth/login", authLimiter, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email, password, first_name, last_name, role, enabled FROM users WHERE email = $1',
      [email]
    );
