...
func CreateBooking(c *gin.Context) {
	...
	// Non-blocking notification: booking creation started
	if err := kafka.PublishBookingStartedNotification(bookingReq); err != nil {
		if ke, ok := err.(*kafka.KafkaError); ok {
			log.Printf("Notification publish failed (non-blocking): %s - %s", ke.Code, ke.Message)
		} else {
			log.Printf("Notification publish failed (non-blocking): %v", err)
		}
	}
